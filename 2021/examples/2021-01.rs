...
// window are marked `A` (`199`, `200`, `208`); their sum is `199 + 200 + 208 = 607`. The second
// window is marked `B` (`200`, `208`, `210`); its sum is `618`. The sum of measurements in the
...
// increases* from the previous sum. So, compare `A` with `B`, then compare `B` with `C`, then `C`
...
fn main() -> Result<()> {
    let input = include_str!("./input/2021-01.txt").trim();
    let depths = input.lines().map(|s| s.parse::<u64>().unwrap()).collect::<Vec<_>>();
    ...
    aoc_lib::set_part_1!(n_increased);

    let mut n_increased = 0;
    let mut last = depths.iter().take(3).fold(0, |r, x| r + x);
    for window in depths.windows(3).skip(1) {
        let total = window.iter().fold(0, |r, x| r + x);
        if total > last {
            n_increased += 1;
        }
    ...
    aoc_lib::set_part_2!(n_increased);
