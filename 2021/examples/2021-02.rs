...
fn main() -> Result<()> {
    let input = include_str!("./input/2021-02.txt")
        .trim()
        .lines()
        .map(|s| s.split(" ").collect::<Vec<_>>())
        .map(|v| (v[0], v[1].parse::<i64>().unwrap()))
        .collect::<Vec<_>>();

    let mut h_pos = 0;
    let mut depth = 0;
    for (cmd, n) in input.clone() {
        match cmd {
            "forward" => h_pos += n,
            "down" => depth += n,
            "up" => depth -= n,
            _ => unreachable!("got: {}", cmd)
        }
    }

    aoc_lib::set_part_1!(h_pos * depth);
    ...
    aoc_lib::set_part_2!(h_pos * depth);
