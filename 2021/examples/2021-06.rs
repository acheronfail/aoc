...
// timer of `4`, and so on until the fifth fish, which has an internal timer of `2`. Simulating
...
fn main() -> Result<()> {
    let input = include_str!("./input/2021-06.txt")
        .trim()
        .split(",")
    ...
    // FIXME: this takes too long for part 2
    for day in 1..=256 {
        ...
        if day == 80 {
            aoc_lib::set_part_1!(fish.len());
        }

        println!("{} ({:?})", day, start.elapsed().unwrap());
    }

    aoc_lib::set_part_2!(fish.len());
