...
//
// A giant [whale][1] has decided your submarine is its next meal, and it's much faster than you
...
//
// [1] https://en.wikipedia.org/wiki/Sperm_whale
//
//
// ## --- Part Two ---
//
...
fn main() -> Result<()> {
    let input = include_str!("./input/2021-07.txt")
        .trim()
        .split(",")
    ...
    for i in 0..*input.iter().max().unwrap() {
        ...
        positions.push((cost, i));
    }

    positions.sort();
    aoc_lib::set_part_1!(positions.first().unwrap().0);
    ...
    positions.sort();
    aoc_lib::set_part_2!(positions.first().unwrap().0);
