...
//
// Santa's reindeer typically eat regular reindeer food, but they need a lot of [magical energy][1]
...
fn main() -> Result<()> {
    let input = include_str!("./input/2022-01.txt").trim();
    let elves = input
        .split("\n\n")
        .map(|items| {
            items
    ...
    aoc_lib::set_part_1!(total_calories.last().unwrap());
    aoc_lib::set_part_2!(total_calories
        .iter()
