...
// storage, a giant [Rock Paper Scissors][1] tournament is already in progress.
//
...
// going to play: `A` for Rock, `B` for Paper, and `C` for Scissors. The second column--" Suddenly,
...
fn main() -> Result<()> {
    let input = include_str!("./input/2022-02.txt").trim();
    let turns = input
        .lines()
        // Rock:     (1) A X
        // Paper:    (2) B Y
    ...
    aoc_lib::set_part_1!(turns.iter().sum::<usize>());
    ...
    aoc_lib::set_part_2!(turns.iter().sum::<usize>());
