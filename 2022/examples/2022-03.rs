...
//
// One Elf has the important job of loading all of the [rucksacks][1] with supplies for the jungle
...
// rucksack is 16 (`p`), 38 (`L`), 42 (`P`), 22 (`v`), 20 (`t`), and 19 (`s`); the sum of these is
// `*157*`.
//
...
// carried by all three Elves*. That is, if a group's badge is item type `B`, then all three Elves
// will have item type `B` somewhere in their rucksack, and at most two of the Elves will be
...
fn item_to_priority(item: char) -> usize {
    ...
    if item.is_ascii_uppercase() {
        return (item as usize) - 38;
    }

    unreachable!();
}

fn str_to_priorities(s: &str) -> HashSet<usize> {
    return s.chars().map(item_to_priority).collect();
}

fn main() -> Result<()> {
    let input = include_str!("./input/2022-03.txt").trim();
    let sum = input
        .lines()
        .map(|items| {
            let (first, second) = items.split_at(items.len() / 2);
            let (first, second) = (str_to_priorities(first), str_to_priorities(second));
    ...
    let sum = groups
        ...
        .map(|lines| {
            ...
            let set = lines
                ...
                .fold(HashSet::from_iter(1..=52), |acc, set| {
                    acc.intersection(&set).cloned().collect::<HashSet<usize>>()
                });
