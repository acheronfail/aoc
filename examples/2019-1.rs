...
// * For a mass of `14`, dividing by 3 and rounding down still yields `4`, so the fuel required is also
// `2`.
...
fn main() {
    ...
    aoc_lib::set_part_2!(input
        .trim()
        .lines()
        .map(|line| ((line.parse::<f64>().unwrap() / 3.0).floor()) as isize - 2)
        .fold(0, |result, module_mass| {
            let mut current_mass = module_mass;
            let mut fuel_mass = 0;
            loop {
                let fuel_required = (current_mass as f64 / 3.0).floor() as isize - 2;
                if fuel_required <= 0 {
                    break;
                }

                fuel_mass += fuel_required;
                current_mass = fuel_required;
            }

            result + module_mass + fuel_mass
        }));
}
