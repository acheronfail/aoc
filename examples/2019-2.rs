...
// "[1202 program alarm][2]". On the radio, an Elf is already explaining how to handle the
// situation: "Don't worry, that's perfectly norma--" The ship computer [bursts into flames][3].
//
// You notify the Elves that the computer's [magic smoke][4] seems to have escaped. "That computer
...
//
// An Intcode program is a list of [integers][5] separated by commas (like `1,0,0,3,99`). To run
// one, start by looking at the first integer (called position `0`). Here, you will find an *opcode*
...
// immediately after an opcode, if any, are called the instruction's *parameters*. For example, in
// the instruction `1,2,3,4`, `1` is the opcode; `2`, `3`, and `4` are the parameters. The
