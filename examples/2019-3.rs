...
// grid, use the [Manhattan distance][1] for this measurement. While the wires do technically cross
// right at the central port where they both start, this point does not count, nor does a wire count
...
// *20*` steps by the first wire and `7+6+4+3 = *20*` steps by the second wire for a total of `20+20
// = *40*` steps.
//
...
fn all_points(line: &str) -> HashMap<(isize, isize), usize> {
    let mut x = 0;
    let mut y = 0;
    let mut length = 0;
    ...
    line.split(',').for_each(|movement| unsafe {
        ...
        for _ in 0..count {
            match &movement.get_unchecked(0..1)[..] {
                "L" => x -= 1,
                "R" => x += 1,
                "U" => y -= 1,
                "D" => y += 1,
...
fn main() -> Result<()> {
    ...
    let make_set = |map: &HashMap<(isize, isize), usize>| {
        map.keys().cloned().collect::<HashSet<(isize, isize)>>()
    };

    let set_a = make_set(&points_a);
    let set_b = make_set(&points_b);
    let intersections = set_a.intersection(&set_b);

    let smallest_from_center = intersections
        .clone()
        .into_iter()
        .map(|(x, y)| x.abs() + y.abs())
        .min()
        .unwrap();
    aoc_lib::set_part_1!(smallest_from_center);

    let smallest_length = intersections
        .into_iter()
        .map(|p| points_a.get(p).unwrap() + points_b.get(p).unwrap())
        .min()
        .unwrap();
    aoc_lib::set_part_2!(smallest_length);
