...
// (your puzzle input). The TEST diagnostic program will run on [your existing Intcode computer][1]
...
// digit, the second parameter's mode is in the thousands digit, the third parameter's mode is in
// the ten-thousands digit, and so on. Any missing modes are `0`.
//
...
// parameter, `3` in immediate mode, simply has value `3`. The result of this operation, `33 * 3 =
// 99`, is written according to the third parameter, `4` in position mode, which also works like it
// did before - `99` is written to address `4`.
//
...
//
// Instead, you'll need to use the TEST to extend the [thermal radiators][1]. Fortunately, the
...
// produce one output:
//
// * `3,9,8,9,10,9,4,9,99,-1,8` - Using *position mode*, consider whether the input is *equal to*
// `8`; output `1` (if it is) or `0` (if it is not).
// * `3,9,7,9,10,9,4,9,99,-1,8` - Using *position mode*, consider whether the input is *less than*
// `8`; output `1` (if it is) or `0` (if it is not).
// * `3,3,1108,-1,8,3,4,3,99` - Using *immediate mode*, consider whether the input is *equal to*
// `8`; output `1` (if it is) or `0` (if it is not).
// * `3,3,1107,-1,8,3,4,3,99` - Using *immediate mode*, consider whether the input is *less than
// *`8`; output `1` (if it is) or `0` (if it is not).
//
