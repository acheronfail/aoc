...
// to reach Santa in time. To do this, you'll need to configure a series of [amplifiers][1] already
...
// should run on your [existing Intcode computer][2]. Each amplifier will need to run a copy of the
// program.
//
// When a copy of the program starts running on an amplifier, it will first use an input instruction
// to ask the amplifier for its current *phase setting* (an integer from `0` to `4`). Each phase
// setting is used *exactly once*, but the Elves can't remember which amplifier needs which phase
// setting.
//
// The program will then call another input instruction to get the amplifier's input signal, compute
// the correct output signal, and supply it back to the amplifier with an output instruction. (If
// the amplifier has not yet received an input signal, it waits until one arrives.)
//
...
// setting amplifier `A` to phase setting `3`, amplifier `B` to setting `1`, `C` to `2`, `D` to `4`,
...
// first signal and the very last signal. To start the process, a `0` signal is sent to amplifier
// `A`'s input *exactly once*.
//
...
fn main() -> Result<()> {
    ...
    let highest_signal = (0..5).permutations(5).fold(0, |result, mut combination| {
        ...
        let mut b = Program::new(int_codes.clone());
        let mut c = Program::new(int_codes.clone());
        let mut d = Program::new(int_codes.clone());
        let mut e = Program::new(int_codes.clone());

        let mut a_input = vec![combination.pop().unwrap(), 0];
        let mut b_input = vec![combination.pop().unwrap()];
        let mut c_input = vec![combination.pop().unwrap()];
        let mut d_input = vec![combination.pop().unwrap()];
        let mut e_input = vec![combination.pop().unwrap()];
        let mut e_output = vec![];

        a.run(&mut a_input, &mut b_input);
        b.run(&mut b_input, &mut c_input);
        c.run(&mut c_input, &mut d_input);
        d.run(&mut d_input, &mut e_input);
        e.run(&mut e_input, &mut e_output);

        std::cmp::max(result, e_output[0])
    });
    aoc_lib::set_part_1!(highest_signal);

    let highest_signal = (5..10).permutations(5).fold(0, |result, mut combination| {
        let mut a_input = vec![combination.pop().unwrap(), 0];
        let mut b_input = vec![combination.pop().unwrap()];
        let mut c_input = vec![combination.pop().unwrap()];
        let mut d_input = vec![combination.pop().unwrap()];
        ...
        loop {
            ...
            c.run(&mut c_input, &mut d_input);
            d.run(&mut d_input, &mut e_input);
            if let StopReason::Halt = e.run(&mut e_input, &mut a_input) {
                break;
            }
        }

        std::cmp::max(result, a_input[0])
    });

    aoc_lib::set_part_2!(highest_signal);
