...
//
// [Your existing Intcode computer][1] is missing one key feature: it needs support for parameters
// in *relative mode*.
//
// Parameters in mode `2`, *relative mode*, behave very similarly to parameters in *position mode*:
// the parameter is interpreted as a position. Like position mode, parameters in relative mode can
...
// it will boost the sensors automatically, but it might take a few seconds to complete the
// operation on slower hardware. In sensor boost mode, the program will output a single value: *the
