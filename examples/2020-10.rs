...
// wrong number of *jolts*. Always prepared, you make a list of all of the joltage adapters in your
// bag.
//
...
// * From your `1`-jolt rated adapter, the only choice is your `4`-jolt rated adapter (difference of
// *`3`*).
...
// (difference of *`1`*).
// * After `12`, only valid adapter has a rating of `15` (difference of *`3`*), then `16` (difference
...
// *more than a trillion* valid ways to arrange them! Surely, there must be an efficient way to count
// the arrangements.
//
...
fn main() {
    ...
    for i in 1..adapters.len() {
        let a = adapters[i - 1];
        let b = adapters[i];
        *diffs.entry(b - a).or_insert(0) += 1
    }

    aoc_lib::set_part_1!(diffs.get(&1).unwrap() * diffs.get(&3).unwrap());
    aoc_lib::set_part_2!(search(&mut HashMap::new(), &adapters, 0));
}

fn search(saved: &mut HashMap<usize, usize>, adapters: &Vec<usize>, idx: usize) -> usize {
    if idx == adapters.len() - 1 {
        1
    } else if let Some(x) = saved.get(&idx) {
        *x
    } else {
        let mut total = 0;
        for i in (idx + 1)..adapters.len() {
            if adapters[i] - adapters[idx] > 3 {
                break;
            }

            total += search(saved, adapters, i);
        }
        saved.insert(idx, total);
        total
    }
}
