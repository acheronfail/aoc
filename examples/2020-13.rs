...
// the sea port at timestamps `0`, `5`, `10`, `15`, and so on. The bus with ID `11` departs at `0`,
// `11`, `22`, `33`, and so on. If you are there when the bus departs, you can ride that bus to the
// airport!
//
...
fn main() -> Result<()> {
    ...
    // tuple of bus_time index, and closest time
    let mut closest = None;
    bus_times.iter().enumerate().for_each(|(i, time)| {
        let mut current = *time;
        while current < depart_time {
            current += time;
        }
        ...
        match closest {
            Some((_, other)) => {
                if current < other {
                    closest = Some((i, current))
                }
            }
            None => closest = Some((i, current)),
        }
    });

    let (idx, bus_depart_time) = closest.unwrap();
    let minutes_waited = bus_depart_time - depart_time;
    aoc_lib::set_part_1!(bus_times[idx] * minutes_waited);

    let bus_times = lines[1]
        .trim()
        .split(',')
        .enumerate()
        .filter_map(|(i, s)| s.trim().parse::<usize>().ok().map(|n| (i, n)))
        .collect::<Vec<(usize, usize)>>();

    // I did not come up with this myself, I was not smart enough for this one :(
    // apparently this has something to do with the Chinese Remainder Theorem...
    let mut start = 0;
    let mut step = 1;
    for (offset, bus_time) in &bus_times {
        let mut time = start;
        loop {
            if (offset + time) % bus_time == 0 {
                start = time;
                step *= bus_time;
                break;
            }
    ...
    loop {
        if bus_times
            .iter()
            .all(|(offset, bus_time)| (time + offset) % bus_time == 0)
        {
            break;
        }
