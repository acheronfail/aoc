...
// brought a small deck of [space cards][1]! You'd like to play a game of *Combat*, and there's even
...
//
// The first round begins with both players drawing the top card of their decks: `9` and `5`. Player
// 1 has the higher card, so both cards move to the bottom of player 1's deck such that `9` is above
// `5`. In total, it takes 29 rounds before a player has all of the cards:
//
// `-- Round 1 --
...
// is worth the value of the card multiplied by 1, the second-from-the-bottom card is worth the
// value of the card multiplied by 2, and so on. With 10 cards, the top card is worth the value on
...
//
// Recursive Combat still starts by splitting the cards into two decks (you offer to play with the
// same starting decks as before - it's only fair). Then, the game consists of a series of *rounds*
// with a few changes:
//
...
// for player 1. Previous rounds from other games are not considered. (This prevents infinite
...
// own deck (again so that the winner's card is above the other card). Note that the winner's card
// might be *the lower-valued of the two cards* if they won the round due to winning a sub-game. If
...
// recursing into a sub-game of Recursive Combat. (For example, if player 1 draws the `3` card, and
// player 2 draws the `7` card, this would occur if player 1 has at least 3 cards left and player 2
...
fn main() -> Result<()> {
    let input = include_str!("./input/2020-22.txt").trim();
    ...
    let mut player_1 = parse(0);
    let mut player_2 = parse(1);

    let winner = loop {
        match (player_1.is_empty(), player_2.is_empty()) {
            (true, false) => break player_2,
            (false, true) => break player_1,
            _ => {}
        }

        match (player_1.remove(0), player_2.remove(0)) {
            (a, b) if a > b => {
                player_1.push(a);
                player_1.push(b);
            }
    ...
    aoc_lib::set_part_1!(calc_score(winner));

    if let Some((_, winner)) = combat(parse(0), parse(1)) {
        aoc_lib::set_part_2!(calc_score(winner));
    } else {
        anyhow::bail!("Failed to solve part 2");
    }

    Ok(())
}

fn combat(mut player_1: Vec<usize>, mut player_2: Vec<usize>) -> Option<(bool, Vec<usize>)> {
    let mut seen = HashSet::new();

    loop {
        match (player_1.is_empty(), player_2.is_empty()) {
            (true, false) => return Some((false, player_2.to_vec())),
            (false, true) => return Some((true, player_1.to_vec())),
            _ => {}
        }

        let key = (player_1.clone(), player_2.clone());
        if seen.contains(&key) {
            return Some((true, player_1.to_vec()));
        }
        seen.insert(key);

        let (a, b) = (player_1.remove(0), player_2.remove(0));
        let player_1_won = if player_1.len() >= a && player_2.len() >= b {
            let (p1_won, _) = combat(
