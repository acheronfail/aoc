...
// support is already on the way! They even created all the room keys this morning; you can take
// yours now and give them your room deposit once the registration system comes back online.
//
// The room key is a small [RFID][1] card. Your room is on the 25th floor and the elevators are also
// temporarily out of service, so it takes what little energy you have left to even climb the stairs
// and navigate the halls. You finally reach the door to your room, swipe your card, and - *beep* -
// the light turns red.
//
...
//
// "Well, it sounds like the card isn't sending the right command to unlock the door. If you go back
// to the check-in desk, surely someone there can reset it for you." Still catching your breath, you
...
// goes off!
//
// "It's an emergency!" the Elf calling you explains. "The [soft serve][1] machine in the cafeteria
// on sub-basement 7 just failed and you're the only one that knows how to fix it! We've already
// dispatched a reindeer to your location to pick you up."
//
// You hear the sound of hooves landing on your balcony.
//
// The reindeer carefully explores the contents of your room while you figure out how you're going
// to pay the *50 stars* you owe the resort before you leave. Noticing that you look concerned, the
// reindeer wanders over to you; you see that it's carrying a small pouch.
//
...
//
// [1] https://en.wikipedia.org/wiki/Soft_serve

use anyhow::Result;
use mod_exp::mod_exp;

fn transform(subject: usize, loop_size: usize) -> usize {
    mod_exp(subject, loop_size, 20201227)

    // NOTE: fast exponentiation algorithm
    // if loop_size == 0 {
    //     1
    // } else if loop_size % 2 == 0 {
    //     transform((subject * subject) % 20201227, loop_size / 2)
    // } else {
    //     (subject * transform(subject, loop_size - 1)) % 20201227
    // }

    // NOTE: naive algorithm
    // let mut result = 1;
...
fn find_loop_size(subject: usize, pubkey: usize) -> usize {
    let mut loop_size = 0;
    while transform(subject, loop_size) != pubkey {
        loop_size += 1;
    }
...
fn main() -> Result<()> {
    let input = include_str!("./input/2020-25.txt").trim();
    let numbers = input
        .lines()
        .map(|line| line.parse::<usize>().unwrap())
        .collect::<Vec<_>>();

    let (card_pubkey, door_pubkey) = (numbers[0], numbers[1]);
    let card_loop_size = find_loop_size(7, card_pubkey);
    let encryption_key = transform(door_pubkey, card_loop_size);

    aoc_lib::set_part_1!(encryption_key);
    aoc_lib::set_part_2!("there is no part 2!");
