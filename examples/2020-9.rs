...
//
// * `26` would be a *valid* next number, as it could be `1` plus `25` (or many other pairs, like `2`
// and `24`).
...
fn main() {
    ...
    for (i, n) in numbers.iter().enumerate().skip(preamble) {
        ...
        if !found {
            part_2_target = *aoc_lib::set_part_1!(n);
            break;
        }
    }

    let numbers = numbers.iter().rev().cloned().collect::<Vec<usize>>();
    let mut start = 0;
    loop {
        let mut nums = vec![];
        let mut end = start;
        let mut total = 0;
        while nums.len() < 2 || total < part_2_target {
            nums.push(numbers[end]);
            end += 1;
            total = nums.iter().sum::<usize>();
        }

        if total == part_2_target {
            nums.sort();
            aoc_lib::set_part_2!(nums[0] + nums[nums.len() - 1]);
            break;
        }
